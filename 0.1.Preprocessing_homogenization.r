...
# MAGIC %md
# MAGIC ##AD – Bangert et al, 2021
# MAGIC - Does not include non-lesional samples.
# MAGIC - Includes treated samples that will be removed
# MAGIC - 5 untreated AD, 5 AD treated for 16 weeks, 4 AD treated for 1 year DB, 4 AD_SH and 4 HC​

# COMMAND ----------

# bangert <- readRDS(file="/dbfs/mnt/sandbox/Rindler/combined.obj2.rds")

# COMMAND ----------

# head(bangert@meta.data,5)

# COMMAND ----------

# DimPlot(bangert, group.by = "CELLTYPE_new", label = TRUE)

# COMMAND ----------

# unique(bangert$CELLTYPE_new)

# COMMAND ----------

# bangert$h_celltype <- gsub("DC[1-3]*", "DC", bangert$CELLTYPE_new)
# bangert$h_celltype <- gsub("TC[1-6]*", "TC", bangert$h_celltype)
# bangert$h_celltype <- gsub("KC[1-5]*", "KC", bangert$h_celltype)
# bangert$h_celltype <- gsub("MastC_other", "MastC", bangert$h_celltype)

# COMMAND ----------

# unique(bangert$h_celltype) # "TC"   "DC"    "Treg"   "MastC_other" "Melanocytes" "KC"   "LC" 

# COMMAND ----------

# MAGIC %md
# MAGIC Regarding to the field "CELLTYPE_new", we could merge to have DC for dendritic cells, TC for T-cells, Treg, Melanocytes, KC for keratinocytes, LC and mastC_others as MastC (mastocytes)
...
liu <- readRDS(file="/dbfs/mnt/sandbox/Liu/Science.rds")

# COMMAND ----------

table(liu$donor_id, liu$Status)
...
liu$h_celltype <- gsub("Th|Tc", "TC", liu$final_clustering)
liu$h_celltype <- gsub("Treg", "Treg", liu$h_celltype)
liu$h_celltype <- gsub("ILC1_3|ILC1_NK|ILC2", "ILC", liu$h_celltype)
liu$h_celltype <- gsub("DC1|MigDC|mDC2|mDC1|mDC3|DC2", "DC", liu$h_celltype)
liu$h_celltype <- gsub("LC_3|LC_1|LC_4|LC_2", "LC", liu$h_celltype)
liu$h_celltype <- gsub("Macro_2|Macro_1", "Macro", liu$h_celltype)
liu$h_celltype <- gsub("Mast_cell", "MastC", liu$h_celltype)
liu$h_celltype <- gsub("Inf_mono|Mono", "Mono", liu$h_celltype)
liu$h_celltype <- gsub(".*DC.*", "DC", liu$h_celltype)

# COMMAND ----------

options(repr.plot.width=1000, repr.plot.height=900)
DimPlot(liu, group.by="h_celltype", label = TRUE)

# COMMAND ----------

unique(liu$h_celltype)

# COMMAND ----------

# MAGIC %md
# MAGIC Here we have TC (t cells),ILC, Treg, NK, DC  , LC    , Macro, MastC, Mono, Plasma  

# COMMAND ----------

#Subset of desired conditions
subliu <- subset(liu, subset= Status %in% desiredR)

# COMMAND ----------

#saveRDS(subliu, file="/dbfs/mnt/sandbox/TFM_PAULA/LIU_PROCESSED_TFM.rds")
...
# MAGIC I need to integrate the 3 datasets to be able to compare what is on each one.
...
liu <- subset(liu, h_celltype %in% subalkon_cells) #relevant cells for the study

# COMMAND ----------

Idents(liu) <- liu$h_celltype
liu_celltype_markers <- FindAllMarkers(liu)

# COMMAND ----------

# Save liu markers to Excel
library(openxlsx)
write.xlsx(liu_celltype_markers, "/dbfs/mnt/sandbox/TFM_PAULA/liu_celltype_markers.xlsx")
...
liu_celltype_markers <- read.xlsx("/dbfs/mnt/sandbox/TFM_PAULA/liu_celltype_markers.xlsx")
...
liu_celltype_markers <- liu_celltype_markers[liu_celltype_markers$p_val_adj < 0.05 & abs(liu_celltype_markers$avg_log2FC) >= 2, ]
...
liu_tc_markers <- liu_celltype_markers[liu_celltype_markers$cluster == "TC", ]
...
liu_treg_markers <- liu_celltype_markers[liu_celltype_markers$cluster == "Treg", ]
...
liu_nk_markers <- liu_celltype_markers[liu_celltype_markers$cluster == "NK", ]
...
treg_common_genes <- intersect(liu_treg_markers$gene, intersect(alkon_treg_markers$gene, reynolds_treg_markers$gene))
tcell_common_genes <- intersect(liu_tc_markers$gene, intersect(alkon_tc_markers$gene, reynolds_tc_markers$gene))
nk_common_genes <- intersect(liu_nk_markers$gene, reynolds_nk_markers$gene)
...
# MAGIC %md
# MAGIC Juanlu Markers: 
# MAGIC Fibroblast: α-smooth muscle actin (α-SMA), fibroblast activation protein (FAP), platelet-derived growth factor receptors (PDGFRα/β)
# MAGIC
# MAGIC CD4 Naive T cell: CD45RA, CCR7, CD62L.
# MAGIC
# MAGIC CD4 Helper T cell: CD4, CD25, CD127.
# MAGIC
# MAGIC CD4 Regulatory T cell: FOXP3, IL2RA (CD25), CTLA4.
# MAGIC
# MAGIC Endothelium cell: CD31, CD34, VE cadherin.
# MAGIC
# MAGIC CD8 Effector Memory: CD8A, CCR7, CD62L
# MAGIC
# MAGIC CD8 Cytotoxic: CD8A, TNF alpha, TBX21.
# MAGIC
# MAGIC CD4 follicular helper T-cell: CD4, CXCR5, ICOS.
# MAGIC
# MAGIC Macrophage: CD14, CD16, CD64.
# MAGIC
# MAGIC Conventional dendritic cell: CD141/BDCA3, CD1c/BDCA1.
# MAGIC
# MAGIC Keratinocyte: LY6A, LCE3C, COL17A1.
# MAGIC
# MAGIC B cell: CD20, CD27, CD43, CD19
# MAGIC
# MAGIC Melanocyte: MITF, TYR, DCT5.
# MAGIC
# MAGIC Pericyte: PDGFRB, ACTA2, CSPG4.
# MAGIC
# MAGIC Mast cell: KIT, FcεRI, CD117.
# MAGIC
# MAGIC NK cell: CD56, CD16, CD3.
# MAGIC
# MAGIC CD4 effector memory T-cell: CD45RO, CCR7, CD62L17.
# MAGIC
# MAGIC Smooth muscle cell: ACTA2, CNN1, TAGLN.
# MAGIC
# MAGIC Plasma B cell: CD19, CD20, CD38.
# MAGIC
# MAGIC Th17 T-cell: CD3, CD4, RORγt.
# MAGIC
# MAGIC Plasmablast: CD19, CD44, CD45
# MAGIC
# MAGIC ILC: CD127, CD161, IL-1R1
...
DotPlot(liu, features = cell_markers, group.by = "h_celltype")
...
merged_seurat <- merge(alkon, y = c(liu, reynolds), add.cell.ids = c("ALKON", "LIU", "REYNOLDS"))


# COMMAND ----------

# Save the merged Seurat object
saveRDS(merged_seurat, file="/dbfs/mnt/sandbox/TFM_PAULA/MERGED_SEURAT_TFM.rds")
