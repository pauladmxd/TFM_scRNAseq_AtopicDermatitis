...
# MAGIC
# MAGIC Ensure cell type homogeneity of the current annotations, to have the same level of granularity 
# MAGIC and the same cell types annotated.
# MAGIC - Integrate scRNA-seq datasets using **Seurat**, STACAS(Andreatta et al., 2024) and **Harmony** to 
# MAGIC ensure comparability across datasets addressing batch effects (final method still to be 
# MAGIC validated).
# MAGIC
# MAGIC **Rationale:** Proper integration across multiple datasets will enhance the robustness of results and help 
# MAGIC account for AD heterogeneity
...
# MAGIC %md
# MAGIC IMPORTANTE: Asegurar que para el PCA y UMAP use los SCTransform 
