...
liu_tcell_kegg <- read.xlsx("/dbfs/mnt/sandbox/TFM_PAULA/Liu/LvsHC/GSEA_individually/res_tcell_0.05_liu.xlsx", sheet =1)

# COMMAND ----------

common_tcell_kegg <- reynolds_tcell_kegg[reynolds_tcell_kegg$ID %in% liu_tcell_kegg$ID & sign(reynolds_tcell_kegg$NES) == sign(liu_tcell_kegg$NES[match(reynolds_tcell_kegg$ID, liu_tcell_kegg$ID)]), ]
common_tcell_kegg

# COMMAND ----------

# MAGIC %md
# MAGIC 6/9 are in common for kegg pathways

# COMMAND ----------

common_tcell_kegg_contrary <- reynolds_tcell_kegg[reynolds_tcell_kegg$ID %in% liu_tcell_kegg$ID & sign(reynolds_tcell_kegg$NES) != sign(liu_tcell_kegg$NES[match(reynolds_tcell_kegg$ID, liu_tcell_kegg$ID)]), ]
common_tcell_kegg_contrary
...
liu_tcell_react <- read.xlsx("/dbfs/mnt/sandbox/TFM_PAULA/Liu/LvsHC/GSEA_individually/res_tcell_0.05_liu.xlsx", sheet =2)

# COMMAND ----------

rows <- min(nrow(liu_tcell_react), nrow(reynolds_tcell_react))

# COMMAND ----------

common_tcell_react <- reynolds_tcell_react[reynolds_tcell_react$ID %in% liu_tcell_react$ID & sign(reynolds_tcell_react$NES) == sign(liu_tcell_react$NES[match(reynolds_tcell_react$ID, liu_tcell_react$ID)]), ]
common_tcell_react
...
common_tcell_react_contrary <- reynolds_tcell_react[reynolds_tcell_react$ID %in% liu_tcell_react$ID & sign(reynolds_tcell_react$NES) != sign(liu_tcell_react$NES[match(reynolds_tcell_react$ID, liu_tcell_react$ID)]), ]
common_tcell_react_contrary
...
# MAGIC %md
# MAGIC 96% of similarity in reactome between reynolds and liu
...
liu_tcell_hall <- read.xlsx("/dbfs/mnt/sandbox/TFM_PAULA/Alkon/GSEA_individually/res_tcell_0.05_alkon.xlsx", sheet=3)

# COMMAND ----------

common_tcell_hallmarks_RL <- reynolds_tcell_hall[reynolds_tcell_hall$ID %in% liu_tcell_hall$ID & sign(reynolds_tcell_hall$NES) == sign(liu_tcell_hall$NES[match(reynolds_tcell_hall$ID, liu_tcell_hall$ID)]), ]
common_tcell_hallmarks_RL

# COMMAND ----------

common_tcell_hallmarks_contrary_RL <- reynolds_tcell_hall[reynolds_tcell_hall$ID %in% liu_tcell_hall$ID & sign(reynolds_tcell_hall$NES) != sign(liu_tcell_hall$NES[match(reynolds_tcell_hall$ID, liu_tcell_hall$ID)]), ]
common_tcell_hallmarks_contrary_RL

# COMMAND ----------

# MAGIC %md
# MAGIC HALLMARK_TNFA_SIGNALING_VIA_NFKB also negative in liu enrichment, as in Alkon but positive in Reynolds

# COMMAND ----------

common_tcell_RL <- list(
  Kegg = common_tcell_kegg,
  Reactome = common_tcell_react,
  Hallmarks1 = common_tcell_hallmarks_RL
)

# COMMAND ----------

write.xlsx(common_tcell_RL, "/dbfs/mnt/sandbox/TFM_PAULA/common_markers/common_GSEA_individually/common_pathways_tcell_RL.xlsx")
